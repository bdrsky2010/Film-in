...
- 맞춤 영화 추천
  - 사용자의 선호 장르를 기반으로 맞춤형 영화 추천 제공
- 배우 및 제작진 상세 정보 확인
  - 배우 및 제작진의 세부 프로필 및 필모그래피 정보 제공
- `본 영화` 및 `볼 영화`를 저장 및 알림 설정
  - 관람 예정 영화에 대한 알림 제공(예: 잠금 화면, 알림 센터, 배너)
- `본 영화` 및 `볼 영화` 필터링 및 날짜별 조회
  - 사용자가 설정한 `본 영화` 및 `볼 영화`를 기준으로 필터링된 데이터 제공
  - 설정된 날짜에 따라 영화 리스트를 날짜별로 조회
- 다국어 지원
    - 이 앱은 한국어, 영어, 일본어를 지원하며, 모든 주요 텍스트 및 영화 정보를 각 언어로 번역
    - 사용자의 설정된 국가에 맞게 언어가 번역되어 출력
<br>

# 주요기술
- `본 영화` 및 `볼 영화`를 저장 및 알림 설정
  - Local Notification을 활용한 푸시 알림 기능 구현
- `본 영화` 및 `볼 영화` 필터링 및 날짜별 조회
