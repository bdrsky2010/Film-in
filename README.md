...
<p align="center"> 
    <img src="./images/Leak_4.png" align="center" width="80%"> 
</p>
...
`sink` 내부에서 `Future`를 사용하게 되면 `Future`는 값 혹은 에러에 대한 값이 방출되는 순간 스트림이 종료되면서 구독이 해제가 되고 `sink`는 이 시점을 감지하여 ViewModel과 같은 객체가 deinit이 되는 순간 구독이 해제가 된다. (Future의 구독 해제 시점이 명확하기 때문에 감지가 가능)

결론적으로는 `Task`를 `Future`로 한 번 래핑하여 처리하였으며
값과 에러를 한 번에 처리하기 위해 `Future`의 값에 해당하는 부분을 Result를 사용하였다.
